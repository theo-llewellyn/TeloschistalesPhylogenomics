...
#concatenate all CDSs
mkdir Single_Copy_Orthologues_10sp_CDS
cd Single_Copy_Orthologues_10sp_CDS
cat /rds/general/project/theollewellynproject/live/funannotate/*_CONCOCT_genes_masked/predict_results/*cds-transcripts.fa > Leca125T_CDS_all.fa 
...
for LINE in $LINES
 do
  ...
  sed -n -e "/$LINE/,/>/p" Leca125T_CDS_all.fa | sed '1b;/>.*/d' | sed "s/>.*/${LINE}/" >> ${PREFIX}_CDS.fa
done
